...
data_processing_targets <- tar_plan(
  ...
  coinf_df = create_coinf_df(pcr_pos, test_summaries),
  ...
  tar_target(map_data_pos_order, 
             prep_map_data_pos_order(pos_final),
...
plot_targets <- tar_plan(
  ...
  # Figure 1 (2 panels)
  fig_coinf_per_order = plot_coinf_per_order(pos_final),
  fig_map_pos_order = plot_map_pos_order(map_data_pos_order),
